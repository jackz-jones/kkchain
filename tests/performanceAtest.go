...
func TestTxsSequenceExecute(t *testing.T) {
	...
	chain, _ := core.GenerateChain(gspec.Config, genesis, pow.NewFaker(), db, 4, func(i int, gen *core.BlockGen) {
		...
		case 0:
			...
			for k := 0; k < number; k++ {
				invokeContractFuncTx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(addr1), contractAddr, big.NewInt(0), initcreateContractGas+1000, new(big.Int).SetInt64(1), inputByteData), signer, key1)
