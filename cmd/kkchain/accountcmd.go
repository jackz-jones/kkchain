...
import (
	...
	"github.com/invin/kkchain/console"
...
var (
	walletCommand = cli.Command{
		Name:      "wallet",
		Usage:     "Manage presale wallets",
		ArgsUsage: "",
		Category:  "ACCOUNT COMMANDS",
		Description: `
    kkchain wallet import /path/to/my/presale.wallet
		...
		Subcommands: []cli.Command{
			{
				...
				Description: `
	kkchain wallet [options] /path/to/my/presale.wallet
...
`,
			},
			{
				...
				Description: `
    kkchain account update <address>
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    kkchain account update [options] <address>
...
`,
			},
			{
				...
				ArgsUsage: "<keyFile>",
				Description: `
    kkchain account import <keyfile>
...
For non-interactive use the passphrase can be specified with the -password flag:
    ...
    kkchain account import [options] <keyfile>
...
func makeAccMgr(ctx *cli.Context) (*accounts.Manager, error) {
	cfg := makeConfig(ctx)

	accman, _, err := config.MakeAccountManager(cfg)
	if err != nil {
		return nil, err
	}
	return accman, nil
...
func accountList(ctx *cli.Context) error {
	accmgr, err := makeAccMgr(ctx)
	if err != nil {
		return err
	}
	var index int
	for _, wallet := range accmgr.Wallets() {
...
func accountUpdate(ctx *cli.Context) error {
	...
	accmgr, _ := makeAccMgr(ctx)
	ks := accmgr.Backends(keystore.KeyStoreType)[0].(*keystore.KeyStore)
